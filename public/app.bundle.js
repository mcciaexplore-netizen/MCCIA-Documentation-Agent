...
-----END ${descriptor}-----`;
  ...
  var transcribeButton = $("#transcribe-button");
  ...
  async function startRecording() {
    ...
    try {
      ...
      const recorderOptions = { audioBitsPerSecond: 48e3 };
  ...
  function chooseFile(file, knownDuration = 0) {
    ...
    $("#upload-detail").textContent = `${(file.size / 1024 / 1024).toFixed(1)} MB \xB7 Ready to transcribe`;
    transcribeButton.disabled = false;
    const selectedFile = file;
    state.durationPromise.then((duration) => {
      if (state.file !== selectedFile || !duration) return;
      $("#upload-detail").textContent = `${(file.size / 1024 / 1024).toFixed(1)} MB \xB7 ${formatRecordingTime(duration * 1e3)} \xB7 Ready to transcribe`;
    });
  }
  ...
  async function uploadRecording(file) {
    ...
    const blob = await uploadRecordingFile(`recordings/${safeName}`, file, {
      ...
      onUploadProgress: ({ percentage }) => {
        setBusy(true, "Uploading securely\u2026", `Sending directly to private storage \xB7 ${Math.round(percentage)}%`);
      }
    });
    return blob.url;
  }
  function renderMetadata(data) {
    ...
    $("#speaker-map").textContent = data.diarizationAvailable === false ? "Groq Whisper does not separate speakers yet. All speech is temporarily labelled Speaker 1; edit names where known. WhisperX diarization is planned for a later update." : data.speakers?.length ? `Detected labels: ${data.speakers.join(", ")}. Edit them directly in the transcript if you know the names.` : "Speaker labels were not available for this recording.";
  }
  async function transcribe() {
    ...
    try {
      ...
      setBusy(
        ...
        longRecording ? "Preparing secure 15-minute chunks, then transcribing them with Groq Whisper." : "Preserving Hindi, Marathi, English, and mixed-language speech with Groq Whisper."
      );
  ...
  async function generate() {
    ...
    try {
      ...
      state.baseDocumentMeta = `Generated with ${data.provider || "Groq"} \xB7 ${data.model}`;
  ...
  function showDocument(document2, label, modelLabel, variant) {
    state.document = document2;
    state.activeVariant = variant;
    $("#document-output").textContent = document2;
  ...
  async function runDocumentTool(action, targetLanguage) {
    ...
    const labels = {
      translate: `Translated \xB7 ${targetLanguage}`,
    ...
    try {
      ...
      showDocument(data.document, labels[action], `Generated with ${data.provider || "Groq"} \xB7 ${data.model}`, action);
  ...
  async function checkHealth() {
    try {
      ...
      $("#fireflies-help").textContent = state.firefliesConfigured ? "Uses the transcript and speaker names already available in Fireflies." : "Setup needed: add FIREFLIES_API_KEY to Vercel to enable meeting-link imports.";
      ...
      element.querySelector("span:last-child").textContent = !data.configured ? "Groq key needed" : !data.blobConfigured ? "Storage setup needed" : "Groq + storage ready";
    } catch {
  ...
  ["dragenter", "dragover"].forEach((event) => uploadZone.addEventListener(event, (e) => {
    e.preventDefault();
    uploadZone.classList.add("dragging");
  }));
  ["dragleave", "drop"].forEach((event) => uploadZone.addEventListener(event, (e) => {
    e.preventDefault();
  ...
  $("#download-button").addEventListener("click", () => {
    ...
    link.download = `mccia-${variant}-${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10)}.md`;
    link.click();
  ...
  $("#download-pdf-button").addEventListener("click", async () => {
    ...
    try {
      ...
      const link = document.createElement("a");
      link.href = URL.createObjectURL(blob);
      link.download = `mccia-${variant}-${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10)}.pdf`;
      link.click();
      URL.revokeObjectURL(link.href);
