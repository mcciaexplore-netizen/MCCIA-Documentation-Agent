...
function isSubstantive(entry) {
  ...
  return !/^(hello|hi|okay|ok|yes|no|thank you|thanks|good (morning|afternoon|evening|day))[.!? ]*$/i.test(entry.text);
}
...
function filenameTitle(filename) {
  const value = clean(filename).replace(/\.[a-z0-9]{2,5}$/i, "").replace(/[_-]+/g, " ");
...
function deadline(text) {
  const match = String(text).match(/\b(today|tomorrow|this week|next week|this month|next month|within\s+\d+\s+(?:hour|day|week|month)s?|by\s+(?:today|tomorrow|[A-Z][a-z]+(?:\s+\d{1,2})?|\d{1,2}[\/.\-]\d{1,2}(?:[\/.\-]\d{2,4})?))\b/i);
...
const actionPattern = /\b(action|assign|follow[- ]?up|need to|needs to|should|must|will|we'll|i'll|going to|next step|send|share|prepare|review|connect|schedule|implement|complete|update|provide)\b/i;
...
function meetingMinutes(input, entries) {
  ...
  const decisions = matching(entries, decisionPattern);
  const actions = matching(entries, actionPattern);
  const questions = uniqueEntries(entries.filter((entry) => /\?$/.test(entry.text) || /\b(pending|open question|to be confirmed|not decided|whether)\b/i.test(entry.text))).slice(0, 10);
  const policies = matching(entries, policyPattern, 8);
  const communications = matching(entries, communicationPattern, 8);
  const nextMeeting = matching(entries, /\b(next meeting|meet again|next session|schedule (?:a|the) meeting)\b/i, 4);
